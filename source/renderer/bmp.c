...
RENDERER_API function_signature(bmp_t, bmp_load, memory_allocator_t* allocator, const char* file_path)
{
	...
	UNUSED_VARIABLE u32 bmp_size = binary_parser_u32();
	...
	UNUSED_VARIABLE u32 raw_bitmap_data_size = binary_parser_u32();
	...
	u8* dst = data;
	const u8* src = REINTERPRET_CONST_CAST(const u8*, parser.bytes);
...
UNUSED_FUNCTION static void DWORD_write(FILE* file, DWORD value)
{
	write(file, &value, 4);
}

UNUSED_FUNCTION static void LONG_write(FILE* file, LONG value)
{
	write(file, &value, 4);
}

UNUSED_FUNCTION static void WORD_write(FILE* file, WORD value)
{
	write(file, &value, 2);
}
