...
RENDERER_API vulkan_render_queue_t* vulkan_render_queue_create(vulkan_renderer_t* renderer, const char* name)
{
	...
	vulkan_render_queue_no_alloc(renderer, name, queue);
	return queue;
}
...
RENDERER_API void vulkan_render_queue_no_alloc(vulkan_renderer_t* renderer, const char* name, vulkan_render_queue_t OUT queue)
{
...
RENDERER_API void vulkan_render_queue_release_resources(vulkan_render_queue_t* queue)
{
	...
	{
		subpass_shader_list_t* list = DEREF_TO(dictionary_get_value_ptr_at(&queue->render_pass_handles, i), subpass_shader_list_t*);
		u32 subpass_count = vulkan_render_pass_pool_get(queue->renderer->render_pass_pool, DEREF_TO(dictionary_get_key_ptr_at(&queue->render_pass_handles, i), vulkan_render_pass_handle_t))->subpass_count;
...
RENDERER_API void vulkan_render_queue_add(vulkan_render_queue_t* queue, vulkan_render_object_t* obj)
{
	...
	for(u32 i = 0; i < pass_count; i++)
	{
		...
		if(!dictionary_contains(&queue->render_pass_handles, &passes[i].handle))
		{
			...
			lists = DEREF_TO(dictionary_get_value_ptr(&queue->render_pass_handles, &passes[i].handle), subpass_shader_list_t*);
		for(u32 j = 0; j < subpass_count; j++)
		{
...
RENDERER_API void vulkan_render_queue_dispatch(vulkan_render_queue_t* queue)
{
	...
	{
		// get the handle
		render_pass_handle_t pass_handle = DEREF_TO(dictionary_get_key_ptr_at(&queue->render_passes, i), vulkan_render_pass_handle_t);
		...
		subpass_shader_list_t* lists = DEREF_TO(dictionary_get_value_ptr_at(&queue->render_pass_handles, i), subpass_shader_list_t*);
		...
		for(u32 j = 0; j < sub_pass_count;)
		{
			...
			{
				vulkan_shader_handle_t shader_handle = DEREF_TO(buf_get_ptr_at(&lists[j], k), vulkan_shader_handle_t);
				...
				// bind pipeline
				vulkan_graphics_pipeline_bind(pipeline, queue->renderer);
				...
				// bind GLOBAL_SET
				vulkan_descriptor_set_bind(&queue->renderer->global_set, queue->renderer, VULKAN_DESCRIPTOR_SET_GLOBAL, pipeline_layout); 
				// bind RENDER_SET
				vulkan_descriptor_set_bind(&pass->render_set, queue->renderer, VULKAN_DESCRIPTOR_SET_RENDER, pipeline_layout);
				// bind SUB_RENDER_SET
				vulkan_descriptor_set_bind(&pass->sub_render_sets[j], queue->renderer, VULKAN_DESCRIPTOR_SET_SUB_RENDER_SET, pipeline_layout);
				...
				for(int l = 0; l < material_count; l++)
				{
					...
					vulkan_material_handle_t material_handle = DEREF_TO(dictionary_get_key_ptr_at(materials, l), vulkan_material_handle_t);
					...
					// bind MATERIAL_SET
					vulkan_descriptor_set_bind(&material->material_set, queue->renderer, VULKAN_DESCRIPTOR_SET_MATERIAL, pipeline_layout);
					...
					render_object_list_t* objects = dictionary_get_value_ptr(materials, l);
					u32 object_count = buf_get_element_count(objects);

					// iterate through each object
					for(int m = 0; m < object_count; m++)
					{
						vulkan_render_object_t* object = DEREF_TO(buf_get_ptr_at(objects, m), vulkan_render_object_t*);

						// bind OBJECT_SET
						vulkan_descriptor_set_bind(&object->object_set, queue->renderer, VULKAN_DESCRIPTOR_SET_OBJECT, pipeline_layout);
