...
RENDERER_API void vulkan_render_pass_pool_destroy(vulkan_render_pass_pool_t* pool)
{
	...
	for(buf_ucount_t i = 0; i < count; i++)
		vulkan_render_pass_destroy(CAST_TO(buf_get_ptr_at(&pool->slots, i), vulkan_render_pass_pool_slot_t*)->render_pass);
}

static void vulkan_render_pass_create_info_deep_free(vulkan_render_pass_create_info* create_info)
{
	// TODO:
}
...
{
	memcpy(dst, src, sizeof(vulkan_resource_descriptor_t));
	if(src->handle.field_count > 0)
	{
		dst->handle.fields = heap_new(struct_field_t, src->handle.field_count);
...
static void vulkan_subpass_create_info_deep_copy(vulkan_subpass_create_info_t* dst, vulkan_subpass_create_info_t* src)
{
	...
	dst->color_attachments = src->color_attachment_count ? heap_new(VkAttachmentReference, src->color_attachment_count) : NULL;
	memcpy(dst->color_attachments, src->color_attachments, sizeof(VkAttachmentReference) * src->color_attachment_count);
	dst->input_attachments = src->input_attachment_count ? heap_new(VkAttachmentReference, src->input_attachment_count) : NULL;
	memcpy(dst->input_attachments, src->input_attachments, sizeof(VkAttachmentReference) * src->input_attachment_count);
	dst->preserve_attachments = src->preserve_attachment_count ? heap_new(VkAttachmentReference, src->preserve_attachment_count) : NULL;
	memcpy(dst->preserve_attachments, src->preserve_attachments, sizeof(VkAttachmentReference) * src->preserve_attachment_count);
			
	dst->sub_render_set_bindings = src->sub_render_set_binding_count ? heap_new(vulkan_resource_descriptor_t, src->sub_render_set_binding_count) : NULL;
...
static void vulkan_render_pass_create_info_deep_copy(vulkan_render_pass_create_info* dst, vulkan_render_pass_create_info_t* src)
{
	memcpy(dst, create_info, sizeof(vulkan_render_pass_create_info_t));
	dst->attachment_descriptions = src->attachment_description_count ? heap_new(VkAttachmentDescription, src->attachment_description_count) : NULL;
	memcpy(dst->attachment_descriptions, src->attachment_descriptions, sizeof(VkAttachmentDescription) * create->attachment_description_count);

	dst->subpasses = heap_new(vulkan_subpass_create_info_t, src->subpass_count);
	memcpy(dst->subpasses, src->subpasses, sizeof(vulkan_subpass_create_info_t) * create->subpass_count);
	for(u32 i = 0; i < src->subpass_count; i++)
		vulkan_subpass_create_info_deep_copy(&dst->subpasses[i], &src->subpasses[i]);

	dst->render_set_bindings = src->render_set_binding_count ? heap_new(vulkan_resource_descriptor_t, src->render_set_binding_count) : NULL;
...
{	
	...
	return CAST_TO(buf_get_ptr_at(&pool->slots, handle), vulkan_render_pass_pool_slot_t*)->handle;
}

static vulkan_render_pass_pool_slot_t* render_pass_pool_get_slotH(vulkan_render_pass_pool_t* pool, vulkan_render_pass_handle_t handle)
{
...
RENDERER_API vulkan_render_pass_handle_t render_pass_pool_create_pass(vulkan_render_pass_pool_t* pool, vulkan_render_pass_create_info_t* create_info)
{
