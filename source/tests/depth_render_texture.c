...
static void initialize(renderer_t* renderer, TEST_DATA(DEPTH_RENDER_TEXTURE)* data)
{
	...
	data->skyboxShaderH = shader_library_create_shader_from_preset(data->slib, SHADER_LIBRARY_SHADER_PRESET_SKYBOX);
	...
	data->shaderH = shader_library_create_shader_from_preset(data->slib, SHADER_LIBRARY_SHADER_PRESET_DIFFUSE_TEST);
...
static void terminate(renderer_t* renderer, TEST_DATA(DEPTH_RENDER_TEXTURE)* data)
...
static void update(renderer_t* renderer, float deltaTime, TEST_DATA(DEPTH_RENDER_TEXTURE)* data)
{	if(kbhit())
	{
		getch();
