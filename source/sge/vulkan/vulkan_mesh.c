...
SGE_API void vulkan_mesh_draw_indexed_instanced_only(vulkan_mesh_t* mesh, u32 instance_count)
{
	...
	/* only issue draw call when there is something to draw */
	if(instance_count > 0)
	{
...
static void _vulkan_mesh_set_index_buffer(vulkan_mesh_t* mesh, vulkan_buffer_t* buffer, VkIndexType vo_type, bool is_internal)
{
...
SGE_API void vulkan_mesh_create_and_set_index_buffer(vulkan_mesh_t* mesh, vulkan_index_buffer_create_info_t* create_info)
{
	...
	_vulkan_mesh_set_index_buffer(mesh, vulkan_buffer_create(mesh->renderer, &buffer_create_info), create_info->index_type, true);
}
