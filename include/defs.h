...
#define DEFS_PRIVATE static

DEFS_PRIVATE int number_get_digit(int number, int index, int num_digits);
DEFS_PRIVATE char digit_to_char(int digit);
DEFS_PRIVATE void convert_int_to_string(int int_value, char* buffer);

DEFS_PRIVATE int number_get_digit(int number, int index, int num_digits)
{
	index++; 
	int _tens = pow(10, num_digits - index);
	int value1 = number / _tens;
	_tens *= 10;
	int value2 = number / _tens;
	value2 *= 10;
	return (value1 - value2);
}
DEFS_PRIVATE char digit_to_char(int digit) {  return 48 + digit; }
DEFS_PRIVATE void convert_int_to_string(int int_value, char* buffer)
{
	int digit_count = 0; 
	int _value = int_value;
	while(_value > 0)
	{
		++digit_count;
		_value /= 10;
	...
	int i = 0;
	for(; i < digit_count; i++)
		buffer[i] = digit_to_char(number_get_digit(int_value, i, digit_count));
	buffer[i] = 0;
}
#endif
