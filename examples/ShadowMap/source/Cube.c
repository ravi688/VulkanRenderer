...
static void loadAndSetup(Cube* this)
{
	...
	material_set_vec4(this->material, "properties.color", VEC4 { 1, 1, 1, 1 });
	mesh3d_t* mesh_data = mesh3d_load("resource/models/Box.obj");
...
void OnStart(Cube)
{
	...
	this->transform = mat4f_scale((vec3f) { 0.5f, 0.5f, 0.5f });
	loadAndSetup(this);
}
